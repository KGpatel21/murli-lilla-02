    ...
    <div>
      ...
      {filtered.length > 0 ? (
        <div className="furniture-mobile-grid" key={active}>
          {filtered.map((p) => (
            <a key={p.title} href={CONTACT.whatsappFurniture} target="_blank" rel="noopener noreferrer" className="furniture-mcard furniture-mcard-anim group block">
              <div className="furniture-mcard-img">
                <Image src={p.img} alt={p.title} fill sizes="(max-width: 520px) 100vw, 33vw" className="object-cover transition-transform duration-500 group-hover:scale-105" />
              </div>
              <div style={{ padding: "16px 18px 18px" }}>
                ...
                <div className="flex items-center" style={{ gap: 8, marginTop: 14 }}>
                  <span style={{ fontSize: 13, color: "#a30000", letterSpacing: "0.02em" }}>ENQUIRE ON WHATSAPP</span>
